...
fn initialize(
    ...
    vault_state.authority = *signer_key;
...
fn withdraw(
    ...
    let vault_data = Vault::from_account_info(vault)?;
    let seeds = &[
            b"vault",
            vault_data.authority.as_ref(),
            &[vault_data.vault_bump],
        ];
    let signer_seeds = &[&seeds[..]];
    // System transfer with PDA signer
    transfer_lamports_signed(vault, user, amount, let signer_seeds = &[&seeds[..]])?;
    let mut vault_state = VaultState::from_account_info(vault_state)?;
...
pub struct VaultAction {
    pub vault_state: Account<,
    pub vault: UncheckedAccount<,
    pub user: Signer<,
    pub authority: SystemAccount<,
    pub system_program: Program<,
}

impl VaultAction {
    pub const DISCRIMINATOR: [u8; 8] = [240, 65, 66, 252, 43, 196, 131, 45];
    ...
    pub fn read(data: &[u8]) -> Result<Self, ProgramError> {
        ...
        let vault_state: Account< = Account<::from_le_bytes(data[offset..offset + 32].try_into().unwrap());
        offset += 32;
        let vault: UncheckedAccount< = UncheckedAccount<::from_le_bytes(data[offset..offset + 32].try_into().unwrap());
        offset += 32;
        let user: Signer< = Signer<::from_le_bytes(data[offset..offset + 32].try_into().unwrap());
        offset += 32;
        let authority: SystemAccount< = SystemAccount<::from_le_bytes(data[offset..offset + 32].try_into().unwrap());
        offset += 32;
        let system_program: Program< = Program<::from_le_bytes(data[offset..offset + 32].try_into().unwrap());
        offset += 32;
        Ok(Self { vault_state, vault, user, authority, system_program })
    }
    ...
    pub fn write(data: &mut [u8], value: &Self) -> ProgramResult {
        ...
        let mut offset = 8usize;
        data[offset..offset + 32].copy_from_slice(&value.vault_state.to_le_bytes());
        offset += 32;
        data[offset..offset + 32].copy_from_slice(&value.vault.to_le_bytes());
        offset += 32;
        data[offset..offset + 32].copy_from_slice(&value.user.to_le_bytes());
        offset += 32;
        data[offset..offset + 32].copy_from_slice(&value.authority.to_le_bytes());
        offset += 32;
        data[offset..offset + 32].copy_from_slice(&value.system_program.to_le_bytes());
        offset += 32;
        Ok(())
    }
