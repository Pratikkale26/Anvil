...
${maybeRead}${prelude.length > 0 ? `${prelude.join("\n")}\n` : ""}    let seeds = &[
   ...
   */
  /**
   * N5 — Pinocchio Pyth modern read. Hand-rolled PriceUpdateV2 byte
   * deserialization. Layout pinned to pyth-solana-receiver-sdk 0.6
   * PriceUpdateV2:
   *   0..8     Anchor discriminator
   *   8..40    write_authority (Pubkey)
   *   40       verification_level tag (0=Partial, 1=Full)
   *   41       num_signatures (only present when tag=0 Partial)
   *   N..      price_message (84 bytes — see legacy comments for layout)
   *   N+84..   posted_slot (u64)
   *
   * Computes the price_message start offset dynamically (41 vs 42)
   * based on the verification_level tag. The PriceFeedMessage layout
   * within the embedded section:
   *   0..32    feed_id ([u8; 32])
   *   32..40   price (i64)
   *   40..48   conf (u64)
   *   48..52   exponent (i32)
   *   52..60   publish_time (i64)
   *   60..68   prev_publish_time (i64)
   *   68..76   ema_price (i64)
   *   76..84   ema_conf (u64)
   *
   * Feed-id cross-check: the emit compares the embedded feed_id
   * against the user-supplied expression. Catches the case where a
   * client passes the wrong feed account — common DeFi attack vector.
   */
  override emitPythReadPriceModern(
    ...
    // Strip leading & from feedIdExpr if present so we compare bytes
    // (source: `&feed_id` arg means a [u8; 32] reference; we want the
    // pointed-to slice).
    ...
    return [
      ...
      `        return Err(ProgramError::InvalidAccountData);`,
      `    }`,
      `    // PriceUpdateV2: 8B Anchor disc + 32B write_authority +`,
      `    // 1-2B verification_level + 84B PriceFeedMessage + 8B posted_slot.`,
      `    // verification_level is a Borsh-encoded enum. Tag values are`,
      `    // documented as 0=Partial(u8 num_signatures), 1=Full. Any tag`,
      `    // > 1 means we're either reading the wrong account or the`,
      `    // upstream layout changed — fail loud rather than read garbage`,
      `    // from the wrong offset (silent wrong-price = money loss).`,
