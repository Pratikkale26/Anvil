...
export function detectCpi(
  ...
  // ── MagicBlock Ephemeral Rollups (ephemeral-rollups-sdk 0.16.2) ──
  // Ordering: the *_with_* names contain shorter supported names as
  // substrings, so unsupported-longest dispatch first. All names are
  // unique across the SPL/MPL/T22 catalogs — no precedence hazard with
  // the blocks below.
  if (
    funcText === "delegate_account_with_actions" || funcText.endsWith("::delegate_account_with_actions") ||
    funcText === "delegate_account_with_any_validator" || funcText.endsWith("::delegate_account_with_any_validator") ||
    funcText === "delegate_account" || funcText.endsWith("::delegate_account") ||
    funcText === "create_schedule_commit_ix" || funcText.endsWith("::create_schedule_commit_ix")
  ) {
    // Raw SDK cpi-module calls (outside the #[delegate]-generated method
    // path). The DelegateAccounts struct-literal plumbing isn't lowered in
    // v1 — refuse loudly rather than emit a half-wired delegation.
    warnMagicBlockUnsupported(collector, funcText, callNode);
    return fallbackPassThrough(callNode);
  }
  if (funcText === "commit_accounts" || funcText.endsWith("::commit_accounts")) {
    return extractMagicBlockCommit(callNode, collector, false);
  }
  if (funcText === "commit_and_undelegate_accounts" || funcText.endsWith("::commit_and_undelegate_accounts")) {
    return extractMagicBlockCommit(callNode, collector, true);
  }
  if (funcText === "undelegate_account" || funcText.endsWith("::undelegate_account")) {
    return extractMagicBlockUndelegate(callNode, collector);
  }
  if (/\.build_and_invoke(_signed)?$/.test(funcText) && callNode.text.includes("MagicIntentBundleBuilder")) {
    return extractMagicBlockIntentBundle(callNode, collector);
  }
  {
    const delegateMethod = /^ctx\s*\.\s*accounts\s*\.\s*delegate_([A-Za-z_]\w*)$/.exec(funcText);
    if (delegateMethod?.[1]) {
      const argsNode = callNode.childForFieldName("arguments");
      // #[delegate]'s generated method always takes (payer, seeds, config)
      // with a DelegateConfig third arg — require it so a user-defined
      // `delegate_*` helper method can't misroute here.
      if (argsNode && /\bDelegateConfig\b/.test(argsNode.text)) {
        return extractMagicBlockDelegate(callNode, collector, delegateMethod[1]);
      }
    }
  }
...
//
// Wire facts (ephemeral-rollups-sdk 0.16.2 source):
//   delegate  → dlp (DELeGG…) u64-LE discriminator 0 (19 = any-validator) +
//               borsh DelegateAccountArgs { commit_frequency_ms: u32,
//               seeds: Vec<Vec<u8>>, validator: Option<Pubkey> }
//   commit    → magic program (Magic111…) bincode enum tag [1,0,0,0]
//               (ScheduleCommit) / [2,0,0,0] (ScheduleCommitAndUndelegate);
//               accounts [payer(s), magic_context(w), fee_vault?(w), …commit]
//   undeleg.  → owner-program callback; SDK validates buffer (signer,
//               dlp-owned, canonical undelegate-buffer PDA), re-creates the
//               PDA via system CreateAccount signed with the seeds+bump,
//               copies buffer data in.

function warnMagicBlockUnsupported(
  collector: WarningCollector | undefined,
  construct: string,
  node: SyntaxNode,
): void {
  collector?.add({
    ...
    message:
      `MagicBlock construct \`${construct}\` is outside Anvil's supported catalog ` +
      `(#[delegate]-generated delegate_<field>() / commit_accounts / ` +
      `commit_and_undelegate_accounts / MagicIntentBundleBuilder commit chains / ` +
      `process_undelegation). Carried as pass-through — deploy is blocked until ` +
      `it's hand-ported.`,
...
/**
 * `ctx.accounts.delegate_<f>(payer, pda_seeds, DelegateConfig { … })` —
 * the method generated by #[delegate] for each `del`-marked field `<f>`.
 * Companion account names are re-derived from the same deterministic
 * naming scheme the pre-parse expansion injects.
 */
function extractMagicBlockDelegate(
  callNode: SyntaxNode,
  collector: WarningCollector | undefined,
  fieldName: string,
): BodyStatement {
  ...
  const args = getArguments(argsNode);
  if (args.length < 3) {
    warnMagicBlockUnsupported(collector, `delegate_${fieldName} (unexpected arg count)`, callNode);
    return fallbackPassThrough(callNode);
  }
  const payer = cleanAccountRef(args[0]!.text.replace(/^&\s*/, ""));
  const seedsExpr = args[1]!.text.trim();
  const configText = args[2]!.text;

  let commitFrequencyMs: string | undefined;
  let validator: string | undefined;
  if (/DelegateConfig\s*::\s*default\s*\(\s*\)/.test(configText)) {
    // both fields default
  } else if (/DelegateConfig\s*\{/.test(configText)) {
    const freq = /commit_frequency_ms\s*:\s*([\s\S]+?)\s*(?:,|\}|\.\.)/.exec(configText);
    if (freq?.[1] && freq[1].trim() !== "") commitFrequencyMs = freq[1].trim().replace(/,$/, "");
    const val = extractStructFieldExpr(configText, "validator");
    if (val && val !== "None") validator = val;
  } else {
    // Variable-bound / computed config — can't lower field-by-field.
    warnMagicBlockUnsupported(collector, `delegate_${fieldName} (non-literal DelegateConfig)`, callNode);
    return fallbackPassThrough(callNode);
  }

  return {
    kind: "cpi_magicblock_delegate",
    pda: fieldName,
    payer,
    buffer: `buffer_${fieldName}`,
    delegationRecord: `delegation_record_${fieldName}`,
    delegationMetadata: `delegation_metadata_${fieldName}`,
...
/** Pull `<field>: <expr>` out of a struct-literal text, respecting nesting. */
function extractStructFieldExpr(structText: string, field: string): string | undefined {
  const start = structText.search(new RegExp(`\\b${field}\\s*:`));
  if (start === -1) return undefined;
  const afterColon = structText.indexOf(":", start) + 1;
  let depth = 0;
  let i = afterColon;
  for (; i < structText.length; i++) {
    const ch = structText[i];
    if (ch === "(" || ch === "[" || ch === "{") depth++;
    else if (ch === ")" || ch === "]" || ch === "}") {
      if (depth === 0) break;
      depth--;
  ...
  const expr = structText.slice(afterColon, i).trim();
  return expr.length > 0 ? expr : undefined;
}

/**
 * `commit_accounts(payer, vec![…], magic_context, magic_program[, fee_vault])`
 * — both the 4-arg (SDK ≤0.6) and 5-arg (SDK ≥0.7, Option<fee_vault>) forms.
 */
function extractMagicBlockCommit(
  callNode: SyntaxNode,
  collector: WarningCollector | undefined,
  undelegate: boolean,
): BodyStatement {
  const name = undelegate ? "commit_and_undelegate_accounts" : "commit_accounts";
  const argsNode = callNode.childForFieldName("arguments");
  if (!argsNode) return fallbackPassThrough(callNode);
  const args = getArguments(argsNode);
  if (args.length < 4) {
    warnMagicBlockUnsupported(collector, `${name} (unexpected arg count)`, callNode);
    return fallbackPassThrough(callNode);
  }
  const vecMatch = /vec!\s*\[([\s\S]*)\]/.exec(args[1]!.text);
  if (!vecMatch?.[1]) {
    warnMagicBlockUnsupported(collector, `${name} (non-literal account vec)`, callNode);
    return fallbackPassThrough(callNode);
  }
  const accounts = splitTopLevelArgs(vecMatch[1])
    .map((a) => cleanAccountRef(a.trim().replace(/^&\s*/, "")))
    .filter((a) => a.length > 0);
  if (accounts.length === 0) {
    warnMagicBlockUnsupported(collector, `${name} (empty account vec)`, callNode);
    return fallbackPassThrough(callNode);
  }
  let magicFeeVault: string | undefined;
  const vaultText = args[4]?.text.trim();
  if (vaultText && vaultText !== "None") {
    const someInner = stripSomeWrap(vaultText);
    magicFeeVault = cleanAccountRef(someInner.replace(/^&\s*/, ""));
  }
  return {
    kind: "cpi_magicblock_commit",
    payer: cleanAccountRef(args[0]!.text.replace(/^&\s*/, "")),
    magicContext: cleanAccountRef(args[2]!.text.replace(/^&\s*/, "")),
    magicProgram: cleanAccountRef(args[3]!.text.replace(/^&\s*/, "")),
    accounts,
    magicFeeVault,
    undelegate,
  };
 ...
 *    [.magic_fee_vault(v)] .commit(&[…]) | .commit_and_undelegate(&[…])
 *    .build_and_invoke()` — the modern (0.7+) commit API. Simple
 * single-intent chains lower to cpi_magicblock_commit; anything with
 * actions / call handlers / signed invoke refuses loudly.
 */
function extractMagicBlockIntentBundle(
  callNode: SyntaxNode,
  collector: WarningCollector | undefined,
): BodyStatement {
  const text = callNode.text;
  const refuse = (why: string): BodyStatement => {
    warnMagicBlockUnsupported(collector, `MagicIntentBundleBuilder (${why})`, callNode);
    return fallbackPassThrough(callNode);
  };
  if (/\.build_and_invoke_signed\s*\($/.test(text) || /\.build_and_invoke_signed\b/.test(text)) {
    return refuse("build_and_invoke_signed");
  }
  if (/\.(call_handler|action|base_action|schedule_task|add_action_callback)\s*\(/.test(text)) {
    return refuse("actions / call handlers");
  }
  const newMatch = /MagicIntentBundleBuilder\s*::\s*new\s*\(([\s\S]*?)\)\s*\./.exec(text);
  if (!newMatch?.[1]) return refuse("unrecognized ::new args");
  const newArgs = splitTopLevelArgs(newMatch[1]).map((a) => cleanAccountRef(a.trim().replace(/^&\s*/, "")));
  if (newArgs.length !== 3) return refuse("unexpected ::new arg count");

  const commitCall = /\.(commit_and_undelegate|commit)\s*\(\s*&\s*\[([\s\S]*?)\]\s*\)/.exec(text);
  if (!commitCall?.[2]) return refuse("no .commit(&[…]) / .commit_and_undelegate(&[…]) leg");
  const accounts = splitTopLevelArgs(commitCall[2])
    .map((a) => cleanAccountRef(a.trim().replace(/^&\s*/, "")))
    .filter((a) => a.length > 0);
  if (accounts.length === 0) return refuse("empty commit account list");

  let magicFeeVault: string | undefined;
  const vault = /\.magic_fee_vault\s*\(([\s\S]*?)\)/.exec(text);
  if (vault?.[1]) magicFeeVault = cleanAccountRef(vault[1].trim().replace(/^&\s*/, ""));

  collector?.add({
    code: "magicblock_intent_bundle_downgraded",
    message:
      "MagicIntentBundleBuilder chain lowered to the classic ScheduleCommit wire on the " +
      "Pinocchio target (the intent-bundle serializer is not vendored). The magic program " +
      "accepts both forms; behavior in the ephemeral rollup is equivalent, but the CPI " +
      "bytes sent to Magic111… differ from the Anchor original on Pinocchio. Native " +
      "target reproduces the builder via ephemeral-rollups-sdk (byte-exact).",
  ...
  return {
    kind: "cpi_magicblock_commit",
    payer: newArgs[0]!,
    magicContext: newArgs[1]!,
    magicProgram: newArgs[2]!,
    accounts,
    magicFeeVault,
    undelegate: commitCall[1] === "commit_and_undelegate",
    viaIntentBundle: true,
  };
}

/**
 * `undelegate_account(…)` inside process_undelegation. Two arg shapes:
 *   Anvil-injected (5): (base_account, buffer, payer, system_program, seeds)
 *   raw SDK       (6): (delegated, &id()/owner_program, buffer, payer,
 *                       system_program, seeds)
 */
function extractMagicBlockUndelegate(
  callNode: SyntaxNode,
  collector: WarningCollector | undefined,
): BodyStatement {
  const argsNode = callNode.childForFieldName("arguments");
  if (!argsNode) return fallbackPassThrough(callNode);
  const args = getArguments(argsNode);
  const a = (i: number) => cleanAccountRef(args[i]!.text.replace(/^&\s*/, ""));
  if (args.length === 5) {
    return {
      kind: "cpi_magicblock_undelegate",
      baseAccount: a(0),
      buffer: a(1),
      payer: a(2),
      systemProgram: a(3),
      seedsArg: args[4]!.text.trim(),
    };
  }
  if (args.length === 6) {
    return {
      ...
      payer: a(3),
      systemProgram: a(4),
      seedsArg: args[5]!.text.trim(),
    };
  }
  warnMagicBlockUnsupported(collector, "undelegate_account (unexpected arg count)", callNode);
  return fallbackPassThrough(callNode);
}
