...
const TRACKED: TrackedCase[] = [
  ...
  // for events files (futarchy).
  {
    ...
    maxErrors: 40,
    reason: "Down from 51 after sibling-crate import strip + body-call commentout (commit pending). Remaining: pass-through `invoke(...)?;` shape drops trailing `?;` causing parse fails, plus AccountInfo field access on sibling state types (Ms.create_key, etc.) — those need typed-state-from-sibling-crate handling.",
  ...
  },
  {
    ...
    source: "Squads-Protocol/squads-mpl (programs/roles)",
    maxErrors: 41,
    reason: "Down from 55 after sibling-crate import strip + body-call commentout + trait-impl commentout (commentOutSiblingTraitImpl). Remaining: AccountInfo field-access on sibling state types (Ms.create_key, etc.) + pass-through invoke(...)?; semicolon drop.",
    repo: "https://github.com/Squads-Protocol/squads-mpl",
    cloneRoot: "/tmp/squads-mpl",
  },
  {
