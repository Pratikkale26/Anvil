...
defineDifferential({
  ...
  callScript: async (svm: LiteSVM, ctx, programId: PublicKey) => {
    ...
    // 1. Allocate + initialize mint, both token accounts, mint to source.
    const setupTx = new Transaction()
      ...
      .add(createInitializeAccountInstruction(ctx.toAta.publicKey, ctx.mint.publicKey, ctx.authority.publicKey))
