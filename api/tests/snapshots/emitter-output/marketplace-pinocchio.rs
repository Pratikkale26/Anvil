...
pub fn initialize(
    ...
    let mut marketplace = Marketplace { admin: [0u8; 32], fee_bps: 0, treasury: [0u8; 32], bump: 0, listing_count: 0 };
...
pub fn list(
    ...
    // anvil: ATA address check: seller_ata
    {
        ...
        let (__expected_ata, _) = pinocchio::pubkey::find_program_address(
            &[seller.key().as_ref(), token_program.key().as_ref(), nft_mint.key().as_ref()],
            &ATA_PROGRAM_ID,
        );
        if seller_ata.key() != &__expected_ata {
    ...
    let mut listing = Listing { seller: [0u8; 32], mint: [0u8; 32], price: 0, seed: 0, bump: 0, marketplace: [0u8; 32], vault: [0u8; 32], is_active: false };
