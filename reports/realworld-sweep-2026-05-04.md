...
|---|---:|---:|---:|
| Parse → IR | 168 | 6 | 96.6% |
| Emit Pinocchio (validator-clean) | 110 | 58 | 65.5% |
| Emit Native (validator-clean) | 124 | 44 | 73.8% |
...
#### `No Anchor #[program] module found` (6)

- `programs/whirlpool/src/lib.rs` (49377 LoC)  
...
- `attribute/program/src/lib.rs` (1652 LoC)  
  → This parser currently supports Anchor entry files. Native multi-file Solana programs like many SPL crates are not transpiled yet.
- `programs/marginfi/src/lib.rs` (27576 LoC)  
  → This parser currently supports Anchor entry files. Native multi-file Solana programs like many SPL crates are not transpiled yet.

## Parser-degradation warnings (loud signal)

Aggregate count across the 168 parsed programs.
...
| Count | Message |
|---:|---|
| 81 | Anvil unsafe-marker (// ⚠️ Anvil … manual rebuild / TODO / not yet supported) — the emit contains a non-functional stub  |
...
| 15 | panic-able .try_into().unwrap() — use .try_into().map_err(\|_\| ProgramError::...)? for safe error propagation. |
| 8 | msg!() macro is not available in the target framework — use pinocchio::log::sol_log() or framework equivalent. |
| 4 | Anchor require!() macro leaked through — should be an if-guard. |
| 4 | Associated constant 'Version::V1' is referenced but not defined in emitted output. |
...
| 3 | Associated constant 'I80F48::ZERO' is referenced but not defined in emitted output. |
| 3 | Associated constant 'I80F48::ONE' is referenced but not defined in emitted output. |
| 3 | Associated constant 'I80F48::MAX' is referenced but not defined in emitted output. |
...
| 3 | Associated constant 'ContractTier::B' is referenced but not defined in emitted output. |
| 2 | Anchor error!() macro leaked through — should use ProgramError::from() or custom error conversion. |
| 2 | Associated constant 'I80F48::MIN' is referenced but not defined in emitted output. |
| 2 | Associated constant 'I80F48::DELTA' is referenced but not defined in emitted output. |
| 2 | Associated constant 'Version::V2' is referenced but not defined in emitted output. |
...
| `bubblegum/program/src/lib.rs` | 6386 | ✓ | 36 | 3 | 16 / 19 | 13 / 20 |
| `programs/drift/src/lib.rs` | 67412 | ✓ | 0 | 0 | 16 / 62 | 13 / 64 |
...
| `programs/mango-v4/src/lib.rs` | 37039 | ✓ | 105 | 2 | 333 / 60 | 327 / 74 |
...
| `programs/marginfi/src/lib.rs` | 27576 | ✗ (No Anchor #[program] module found) | - | - | - | - |
...
| `programs/openbook-v2/src/lib.rs` | 8374 | ✓ | 29 | 2 | 35 / 31 | 33 / 33 |
...
| `programs/uxd/src/lib.rs` | 8278 | ✓ | 23 | 2 | 94 / 10 | 94 / 12 |
| `programs/whirlpool/src/lib.rs` | 49377 | ✗ (No Anchor #[program] module found) | - | - | - | - |
