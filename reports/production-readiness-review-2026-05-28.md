...
**Flagged residual I did NOT fully verify:** the end-of-fn save skips `isOptional` (walker.ts:2275). Optional state accounts *appear* to be saved **inline** (walker.ts:1940) or stubbed (`optional_accounts_unsupported`), so I believe there is no silent `let mut` + no-save path — but I did **not** exhaustively trace every optional-account flow. **RESOLVED (verified 2026-05-29):** an optional mutable state account that IS mutated gets the whole body **stubbed with `unimplemented!()`** + a loud `⚠️ Anvil` marker + an `optional_accounts_unsupported` warning (repro confirmed) — it never reaches the `let mut` + no-save flow, so there is no silent path. The `isZeroCopy` skip is safe by design (bytemuck writes through the buffer).
...
### Parser claims (verified same session)

- **first-`declare_id`-wins → wrong program ID: CONFIRMED + FIXED** (commit `efe2387`). `extractProgramId` returns the first `declare_id!` in source order regardless of `#[cfg]` (verified: a dual-branch mainnet/devnet fixture silently picked the first, no warning). A genuine *silent-at-transpile* issue (wrong program ID; surfaces at deploy/runtime). Fixed with a non-escalated `multiple_declare_id` warning that surfaces the picked ID — conservative (doesn't change *which* ID is picked, which is ambiguous for a transpiler; just makes it loud).
- **feature-flag "silent drop": REFUTED.** Single-file `/parse` *includes* `#[cfg(feature=…)]` items (no drop — inactive-feature code is kept, a minor divergence), no warning. The project path strips-but-*warns* (`cfg_gated_item_dropped`). Neither silently drops mainnet logic.
- **name-based seed classification** (body-classifier.ts:50-67; consumers :900 / :995): **VERIFIED not silent** (code-based). The name heuristic only decides whether a seed-prep `let` is *optimized* into a typed `pda_signer_seeds` node (names `seeds`/`*_seeds`) or **passed through verbatim** (any other name). Verbatim pass-through preserves the seed array as-written → same PDA (correct), or fails to compile (loud); it does **not** silently derive a wrong PDA. The misclassification → undefined-identifier risk is exactly what the verbatim-passthrough design (+ `hasUserSeedsManagementSignal`) avoids (body-classifier.ts:893-898); the consuming CPI is either recognized or loudly `cpi_custom`-stubbed.

**Verification complete:** across all emitter + parser "silent miscompile / wrong-IR" claims, the **only genuine silent bug was `first-declare_id-wins`** (fixed, `efe2387`). Everything else is compile-loud, already-guarded, or already-fixed. The real *residual* risks are loud/narrow (`invoke_signed` for hand-rolled PDA creates, 8-byte disc) or genuinely-silent-but-corpus-guarded (version-coupled MPL/anchor-spl/Pyth constants — B3/#7, the one worth hardening). The `isOptional` save-skip is verified safe too: optional state accounts are stubbed with `unimplemented!()` + a loud marker + an `optional_accounts_unsupported` warning, so they never reach the `let mut` + no-save flow.

### B6 scoping decision (2026-05-29)

Scoped, **not** implemented — neither half fits a tight + differential-verifiable slice:
- **Non-8-byte discriminator dispatch — DECLINED.** Near-zero value for Anchor transpilation (Anchor is 8-byte by construction; the `#[instruction(discriminator=…)]` override is rare 1.0+) and not differential-verifiable (a non-8-byte-disc Anchor *reference* isn't practically buildable). Narrow + unverifiable — not worth the parser+emitter change.
- **`Option<T>` accounts — real value, but a multi-session arc, not a tonight slice.** Full support spans account-meta + body + `.key()`/`.lamports()`/CPI/init/has_one. A *partial* impl is **more dangerous than the current loud `unimplemented!()` stub** — a wrong optional-account meta position → wrong account at runtime (the exact silent-miscompile class). Do it fixture-first, one surface at a time under the differential gate.
