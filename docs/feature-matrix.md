...
| MPL Core asset lifecycle (CreateV2 + UpdateV2 + TransferV1 + BurnV1) + CreateCollectionV2 (kinobi fluent builder) | Y (parser + hand-rolled bytes; byte-equal differential pending fixture) | Y |
| MPL Core plugin family CPIs (AddPluginV1 / RemovePluginV1 / UpdatePluginV1 / Approve / Revoke) | lint | lint |
...
- **MPL Core** (the newer Metaplex format, separate from Token Metadata). **Full asset lifecycle + collection creation supported**: CreateV2 + UpdateV2 + TransferV1 + BurnV1 + CreateCollectionV2 (kinobi fluent CpiBuilder chains → hand-rolled Borsh args + 4/6/7/8-account metas with MPL_CORE_ID readonly fallback for None optional slots; v1 scope: plugins / external_plugin_adapters / new_update_authority / compression_proof always None). Plugin family CPIs (AddPluginV1 / RemovePluginV1 / UpdatePluginV1 / Approve / Revoke) remain lint-only — the nested Plugin enum's Borsh shape is a separate research arc. Token Metadata IS also fully supported (12 IR kinds, byte-equal differential-gated).
