...
| `declare_program!` cross-program CPI (`<crate>::cpi::<fn>(CpiContext::new[_with_signer](prog, Accounts{…}), args)`) | Y — aliased (`use X::cpi::fn`) + qualified (`X::cpi::fn`) call forms; `invoke` AND `invoke_signed` (PDA-signed via `CpiContext::new_with_signer`); String / fixed-width int / bool / pubkey args; metas driven from the CpiContext struct + the IDL's account flags/order/discriminator. Byte-equal differential-gated (hand→lever, external::update u32, config bool+pubkey, vault PDA-signed invoke_signed). IDL collected from the project `idls/` dir on all /parse paths. Composite-account params + other arg types (bytes/Vec/struct/option/array) loud-refuse — see Known gaps. | Y (same) |
...
- **`declare_program!` CPI — composite-account params and non-{String,int,bool,pubkey} arg types.** Loud-refuse (fail-closed), never silently mis-emitted. Composite-account values (a nested `<crate>::cpi::accounts::*` struct as a field) and arg types like `bytes`/`Vec`/defined-structs/`Option`/arrays each need their own byte-equal fixture before they leave the refuse path. (PDA-signed `CpiContext::new_with_signer` is now supported — see the declare_program! row — using the same scalar-bump + inline `&[bump]` seeds-prep the SPL-CPI-with-signer demos use.)
