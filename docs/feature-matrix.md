...
| `declare_program!` cross-program CPI (`<crate>::cpi::<fn>(CpiContext::new[_with_signer](prog, Accounts{…}), args)`) | Y — aliased (`use X::cpi::fn`) + qualified (`X::cpi::fn`) call forms; `invoke` AND `invoke_signed` (PDA-signed via `CpiContext::new_with_signer`); String / fixed-width int / bool / pubkey / bytes (Vec<u8>) / Option<supported> args; metas driven from the CpiContext struct + the IDL's account flags/order/discriminator. composite-account params (a nested `#[derive(Accounts)]` struct as a field, recursively flattened on both the IDL leaves and the caller's nested CpiContext struct). Byte-equal differential-gated (hand→lever, external::update u32, config bool+pubkey, vault PDA-signed invoke_signed, composite update_composite, blob bytes, option Some(u64)). IDL collected from the project `idls/` dir on all /parse paths. Remaining arg types (Vec&lt;non-u8&gt;/defined-struct/array) loud-refuse — see Known gaps. | Y (same) |
...
- **`declare_program!` CPI — Vec&lt;non-u8&gt; / defined-struct / Option / array arg types.** These loud-refuse (fail-closed), never silently mis-emitted — each needs its own byte-equal fixture before it leaves the refuse path. (String, fixed-width ints, bool, pubkey, bytes/Vec&lt;u8&gt;, PDA-signed `CpiContext::new_with_signer`, and composite-account params are all supported — see the declare_program! row.)
