...
| MPL Core CreateV2 + UpdateV2 + TransferV1 + BurnV1 — full asset lifecycle (kinobi fluent builder) | Y (parser + hand-rolled bytes; byte-equal differential pending fixture) | Y |
| MPL Core CPIs (plugin family + CreateCollectionV2) | lint | lint |
...
- **MPL Core** (the newer Metaplex format, separate from Token Metadata). **Full asset lifecycle supported**: CreateV2 + UpdateV2 + TransferV1 + BurnV1 (kinobi fluent CpiBuilder chains → hand-rolled Borsh args + 6/7/8-account metas with MPL_CORE_ID readonly fallback for None optional slots; v1 scope: plugins / external_plugin_adapters / new_update_authority / compression_proof always None). Plugin family CPIs (AddPluginV1 / RemovePluginV1 / UpdatePluginV1 / Approve / Revoke) and CreateCollectionV2 remain lint-only and pull a TODO stub at emit. Token Metadata IS also fully supported (12 IR kinds, byte-equal differential-gated).
