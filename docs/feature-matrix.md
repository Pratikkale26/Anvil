...
| `declare_program!` cross-program CPI (`<crate>::cpi::<fn>(CpiContext::new(prog, Accounts{…}), args)`) | Y — aliased (`use X::cpi::fn`) + qualified (`X::cpi::fn`) call forms; String / fixed-width int / bool / pubkey args; metas driven from the CpiContext struct + the IDL's account flags/order/discriminator. Byte-equal differential-gated (hand→lever, external::update u32, config bool+pubkey). IDL collected from the project `idls/` dir on all /parse paths. new_with_signer (PDA-signed) + composite-account params + other arg types (bytes/Vec/struct/option/array) loud-refuse — see Known gaps. | Y (same) |
...
- **`declare_program!` CPI — PDA-signed (`CpiContext::new_with_signer`), composite-account params, and non-{String,int,bool,pubkey} arg types.** All loud-refuse (fail-closed), never silently mis-emitted. new_with_signer is deferred on a shared bumps-emit limitation (`let bump = [ctx.bumps.X]` array seeds-prep emits a scalar `u8`, breaking `&[…, &bump]` → E0308); composite-account values (a nested `<crate>::cpi::accounts::*` struct as a field) and arg types like `bytes`/`Vec`/defined-structs/`Option`/arrays each need their own byte-equal fixture before they leave the refuse path.
