...
- **Nested variable-length account-state fields.** A state-account field whose type is a defined struct (or fixed array of one) that *transitively* contains a `String` / `Vec` / `Option` / complex-enum member has a content-dependent on-disk length. Anvil's account read/write tracks the byte offset with a constant size for the struct branch, which would desync the cursor and silently corrupt every following field on Anchor interop — so this shape **loud-refuses** (`unimplemented!("anvil: …")`, flagged by the validator) rather than emit a wrong byte layout. Top-level `String`/`Vec`/`Option` fields are fully supported (they use a variable-length Borsh read that advances by exactly the consumed bytes); the gap is only the *nested* case. A faithful variable-length nested read/write is a separate, larger change to the offset machinery.
