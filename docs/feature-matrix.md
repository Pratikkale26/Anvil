...
| MagicBlock Ephemeral Rollups (`ephemeral-rollups-sdk` 0.16.2): `#[ephemeral]`/`#[delegate]`/`#[commit]` macro expansion, `delegate_<field>()`, `commit_accounts` / `commit_and_undelegate_accounts` (4- and 5-arg forms), simple `MagicIntentBundleBuilder` commit chains, `process_undelegation` callback (external discriminator `[196,28,41,206,48,37,51,167]` preserved) | Y (vendored pinocchio-0.9 port of `ephemeral-rollups-pinocchio` — upstream is pinocchio 0.10-only; intent-bundle chains lower to the classic ScheduleCommit wire, surfaced as `magicblock_intent_bundle_downgraded`. **Delegate leg byte-equal differential-gated against the mainnet `dlp.so` in LiteSVM** — delegated PDA + delegation record + metadata + closed buffer all byte-compared vs the Anchor/real-SDK reference. Commit/undelegate cargo-gated: the magic program exists only inside the ephemeral validator) | Y (wraps the real `ephemeral-rollups-sdk` crate, `backward-compat` feature — byte-exact incl. intent bundles) |
| MagicBlock out-of-catalog (post-delegation actions, `#[ephemeral_accounts]`, `#[action]`, `build_and_invoke_signed`, `MagicInstructionBuilder`, session keys, vrf) | loud-refuse (`magicblock_unsupported`, promoted to validator error) | same |
...
- **Compressed-NFT / state-compression CPIs** (`mpl_bubblegum`, `spl_account_compression`, `spl_noop`). **Transpilable via the standard `declare_program!` + IDL path** — declaring `declare_program!(spl_account_compression)` / `declare_program!(mpl_bubblegum)` with the crate's `idls/<crate>.json` and calling `<crate>::cpi::<fn>(...)` routes through the same external-CPI machinery that's byte-equal-gated for other declared programs (see the `declare_program!` row above). A **raw-crate CPI with NO IDL** is loud-refused (`cnft_compression_unsupported`, promoted to a validator error, pointing the user at the IDL path) since the target instruction layout is unknown. Both are now **byte-equal differential-gated**: `differential-compression-append.test.ts` (a real `spl_account_compression::cpi::append`) and `differential-bubblegum-create-tree.test.ts` (a real `mpl_bubblegum::cpi::create_tree`, which itself CPIs compression + noop — the full cNFT program stack) are byte-compared against Anchor's own `declare_program!` output, running the mainnet `bubblegum.so` + `spl_account_compression.so` + `spl_noop.so` (committed under `tests/fixtures/programs/`) in LiteSVM with real concurrent-Merkle-tree / TreeConfig mutations.
