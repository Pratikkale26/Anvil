...
| `declare_program!` cross-program CPI (`<crate>::cpi::<fn>(CpiContext::new[_with_signer](prog, Accounts{…}), args)`) | Y — aliased (`use X::cpi::fn`) + qualified (`X::cpi::fn`) call forms; `invoke` AND `invoke_signed` (PDA-signed via `CpiContext::new_with_signer`); String / fixed-width int / bool / pubkey / bytes (Vec<u8>) / Option<supported> / Vec<supported> / [T; N] / defined-struct (the external struct def is generated from the IDL `types` so the caller can deserialize it) args — i.e. every arg type except defined-enums; metas driven from the CpiContext struct + the IDL's account flags/order/discriminator. composite-account params (a nested `#[derive(Accounts)]` struct as a field, recursively flattened on both the IDL leaves and the caller's nested CpiContext struct). Byte-equal differential-gated (hand→lever, external::update u32, config bool+pubkey, vault PDA-signed invoke_signed, composite update_composite, blob bytes, option Some(u64), collections Vec&lt;u64&gt;+[u8;4], defined-struct, array [u64;3]). IDL collected from the project `idls/` dir on all /parse paths. Only defined-ENUM args still loud-refuse — see Known gaps. | Y (same) |
...
- **`declare_program!` CPI — defined-ENUM args.** A CPI arg whose type is a defined enum loud-refuses (fail-closed) — only struct defined-types are generated/encoded today; an enum needs variant-discriminant Borsh encoding + a fixture. This is the one remaining unsupported arg shape. Everything else is supported (see the declare_program! row): String, fixed-width ints, bool, pubkey, bytes/Vec&lt;u8&gt;, Vec&lt;supported&gt;, [T; N] (u8 fast-path + per-element for others), Option&lt;supported&gt;, defined-structs (with external struct-def generation), PDA-signed `CpiContext::new_with_signer`, and composite-account params.
