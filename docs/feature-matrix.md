...
- **`declare_program!` CPI — non-{String,int,bool,pubkey} arg types.** Arg types like `bytes`/`Vec`/defined-structs/`Option`/arrays loud-refuse (fail-closed), never silently mis-emitted — each needs its own byte-equal fixture before it leaves the refuse path. (PDA-signed `CpiContext::new_with_signer` and composite-account params are now supported — see the declare_program! row.)
