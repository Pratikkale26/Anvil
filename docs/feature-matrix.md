...
| `declare_program!` cross-program CPI (`<crate>::cpi::<fn>(CpiContext::new[_with_signer](prog, Accounts{…}), args)`) | Y — aliased (`use X::cpi::fn`) + qualified (`X::cpi::fn`) call forms; `invoke` AND `invoke_signed` (PDA-signed via `CpiContext::new_with_signer`); String / fixed-width int / bool / pubkey / bytes (Vec<u8>) / Option<supported> / Vec<supported> / [u8; N] args; metas driven from the CpiContext struct + the IDL's account flags/order/discriminator. composite-account params (a nested `#[derive(Accounts)]` struct as a field, recursively flattened on both the IDL leaves and the caller's nested CpiContext struct). Byte-equal differential-gated (hand→lever, external::update u32, config bool+pubkey, vault PDA-signed invoke_signed, composite update_composite, blob bytes, option Some(u64), collections Vec&lt;u64&gt;+[u8;4]). IDL collected from the project `idls/` dir on all /parse paths. Remaining arg types (defined-struct, non-u8 fixed arrays) loud-refuse — see Known gaps. | Y (same) |
...
- **`declare_program!` CPI — defined-struct args and non-u8 fixed arrays.** These loud-refuse (fail-closed), never silently mis-emitted. A defined-struct arg is blocked on a deeper piece: the CALLER must deserialize the external struct, which needs Anvil to emit the struct definition from the IDL `types` (the CPI-side encoding is already in place); a non-u8 `[T; N]` needs per-element iteration + a fixture. (String, fixed-width ints, bool, pubkey, bytes/Vec&lt;u8&gt;, Vec&lt;supported&gt;, [u8; N], Option&lt;supported&gt;, PDA-signed `CpiContext::new_with_signer`, and composite-account params are all supported — see the declare_program! row.)
