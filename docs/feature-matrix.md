...
- **Compressed-NFT / state-compression CPIs** (`mpl_bubblegum`, `spl_account_compression`, `spl_noop`). Loud-refused by design (`cnft_compression_unsupported`, promoted to a validator error): cNFT operations mutate a concurrent-Merkle-tree account whose correctness can only be proven byte-equal against the real `spl_account_compression` / `spl_noop` / `bubblegum` programs, and there is no loadable `.so` for any of them in the differential harness — so byte-equal (Anvil's trust gate) is unbuildable. Keep these instructions on Anchor. Reopening requires committing those three `.so` fixtures and a post-append tree-account byte-equal test.
