...
| MPL Core CreateV2 + UpdateV2 (kinobi fluent builder) | Y (parser + hand-rolled bytes; byte-equal differential pending fixture) | Y |
| MPL Core CPIs (Transfer / Burn / plugins / collection mint) | lint | lint |
...
- **MPL Core** (the newer Metaplex format, separate from Token Metadata). **CreateV2 + UpdateV2 fully supported** (kinobi fluent `CreateV2CpiBuilder` / `UpdateV2CpiBuilder` chains → hand-rolled Borsh args + 7/8-account meta with MPL_CORE_ID readonly fallback for None optional slots; v1 scope: plugins / external_plugin_adapters / new_update_authority always None). Transfer / Burn / plugin CPIs (TransferV1 / BurnV1 / AddPluginV1 / RemovePluginV1 / UpdatePluginV1 / Approve / Revoke / CreateCollectionV2) remain lint-only and pull a TODO stub at emit. Token Metadata IS fully supported (12 IR kinds, byte-equal differential-gated).
