...
| MPL Core CreateV2 + UpdateV2 + TransferV1 (kinobi fluent builder) | Y (parser + hand-rolled bytes; byte-equal differential pending fixture) | Y |
| MPL Core CPIs (Burn / plugins / collection mint) | lint | lint |
...
- **MPL Core** (the newer Metaplex format, separate from Token Metadata). **CreateV2 + UpdateV2 + TransferV1 fully supported** (kinobi fluent `CreateV2CpiBuilder` / `UpdateV2CpiBuilder` / `TransferV1CpiBuilder` chains → hand-rolled Borsh args + 7/8-account meta with MPL_CORE_ID readonly fallback for None optional slots; v1 scope: plugins / external_plugin_adapters / new_update_authority / compression_proof always None). Burn / plugin CPIs (BurnV1 / AddPluginV1 / RemovePluginV1 / UpdatePluginV1 / Approve / Revoke / CreateCollectionV2) remain lint-only and pull a TODO stub at emit. Token Metadata IS fully supported (12 IR kinds, byte-equal differential-gated).
