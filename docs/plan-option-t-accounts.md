...
**Next — `.key()` / `.lamports()` on an optional (harder).** Inside `if let Some(cfg) = …`, surface 2 deserializes `cfg` to the typed `Config` struct, which has **no** `.key()`/`.lamports()`. Anchor's `Some(cfg)` is `&Account<Config>` which derefs to expose `.key()`. So `cfg.key()` must route to the underlying `AccountInfo` (the `cfg_account` binding), not the deserialized struct. ACTION: in the if-let-Some handler, rewrite `cfg.key()` / `cfg.lamports()` (and similar AccountInfo-level calls) to `cfg_account.key()` etc. Then: optional in CPI args / `init` / `has_one`; finally un-gate (drop `B6_OPTION_T`, un-skip fixture, remove the stub for covered shapes).
